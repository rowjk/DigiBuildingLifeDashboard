...
with st.sidebar:
    ...
    show_pet = st.checkbox("啟用畫面虛擬寵物 (皮卡丘)", value=True)
...
# ----------------- Virtual Pet (Pikachu) Walk Section -----------------
if show_pet:
    pikachu_gif_path = "pikachu.gif"
    if os.path.exists(pikachu_gif_path):
        try:
            with open(pikachu_gif_path, "rb") as f:
                pikachu_b64 = base64.b64encode(f.read()).decode("utf-8")
        except Exception:
            pikachu_b64 = ""
    else:
        pikachu_b64 = ""
    ...
    if pikachu_b64:
        # Parent page style override to make iframe fixed, full-viewport and click-through
        st.markdown("""
        <style>
        iframe[srcdoc*="pikachu-walk-container"] {
        ...
        <head>
            ...
            <style>
                ...
                #pikachu {{
                    ...
                    cursor: pointer; user-select: none; z-index: 999999;
        ...
        <body>
            <div id="pikachu-walk-container">
                <img id="pikachu" src="data:image/gif;base64,{pikachu_b64}" alt="Pikachu">
            </div>
            ...
            <script>
                ...
                function getBoundary() {{
                    return {{
                        maxX: window.innerWidth - pikachu.clientWidth,
                        maxY: window.innerHeight - pikachu.clientHeight
                    }};
                }}
                function walkAround() {{
                    ...
                    const currentX = parseFloat(pikachu.style.left) || 0;

                    if (nextX < currentX) {{ pikachu.classList.add('flip'); }} 
                    else {{ pikachu.classList.remove('flip'); }}

                    pikachu.style.left = nextX + 'px';
                    pikachu.style.top = nextY + 'px';

                    setTimeout(walkAround, Math.random() * 2500 + 2000);
                }}
                window.onload = () => {{
                    pikachu.style.left = '100px';
                    pikachu.style.top = '100px';
                    setTimeout(walkAround, 500);
                }};
                window.onresize = () => {{
                    const {{ maxX, maxY }} = getBoundary();
                    const currentX = parseFloat(pikachu.style.left) || 0;
                    const currentY = parseFloat(pikachu.style.top) || 0;
                    if (currentX > maxX) pikachu.style.left = maxX + 'px';
                    if (currentY > maxY) pikachu.style.top = maxY + 'px';
                }};
            </script>
        </body>
        </html>
        """
        st.components.v1.html(pikachu_html, height=0)
