...
3. **API 容錯與 Mock 資料降級機制**：
   * 系統會嘗試異步獲取台北市交通 VD（車流速限）及國道 VD 與即時事件資料。
   * 若發生連線超時、DNS 解析失敗（如本機環境下官方網域 `tcgdata.taipei` 無法解析）或斷網時，系統將啟動高擬真降級機制，結合當前時間段生成合理的車速數值，並為週邊路段隨機觸發交通事件（如車輛故障、施工或車禍追撞）。
4. **即時車速與突發事件雙欄式卡片化排版**：
   * **左側「週邊道路車流速限」**：展示附近各觀測點的車速，並依據車速區間自動判定順暢度級別，標註紅（壅塞）、黃（車多）、綠（順暢）三色，視覺呈現一目了然。
   * **右側「即時路況突發事件」**：若週邊路段發生突發事件，會以紅/黃邊框的警示卡片呈現詳細的文字描述（如佔用車道、回堵長度等）；若無事件，則會顯示綠色的「週邊路況良好」卡片，維持介面的平衡美觀。
